<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <div class="row">
        <div class="col">
            <div class="card">
              <div class="card-body">
              <a href="<?= base_url('pembelian') ?>" class="btn btn-warning" id="tmbh">Tambah Pembelian Modal</a>
              </div>
            </div>
        </div>
      </div> 
                    ...
                    foreach($pembelian as $row):
                        ?>
                            <tr>
                                ...
                                <td><?= substr($row->tgl_pembelian,0,10);?></td>
                                <td><?= $row->jumlah_pembelian;?></td>
                                <td><?= rupiah($row->total_pembelian);?></td>
                                <td><a onclick="deleteConfirm('<?php echo base_url('pembelian/deletepembelian/'.$row->id_transaksi) ?>')" href="#" class="btn btn-danger" role="button" aria-pressed="true">Hapus</a></td>
                            </tr>
                        <?php
                    endforeach;    
                ?>
...
<script>
          function deleteConfirm(url){
              url2 = "<?= base_url('pembelian/Listpembelian') ?>"; //diisi dengan halaman ini
              ...
              var myModal = new bootstrap.Modal(document.getElementById('deletepembelian'), {  keyboard: false });
